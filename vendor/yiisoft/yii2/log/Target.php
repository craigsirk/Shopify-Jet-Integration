...
abstract class Target extends Component
{
    ...
    public $messages = [];
    /**
     * Added in Shopify Jet App to log the store in the DB
     * @var unknown
     */
    public $store;
    ...
    /**
     * Returns a string to be of the store to the given message.
     * If [[store]] is configured it will return the result of the callback.
     * The default implementation will return null.
     * @param array $message the message being exported.
     * The message structure follows that in [[Logger::messages]].
     * @return string the store string
     */
    public function getMessageStore($message)
    {
    	if ($this->store !== null) {
    		return call_user_func($this->store, $message);
    	}
    	/* @var $session \yii\web\Session */
    	$session = Yii::$app->has('session', true) ? Yii::$app->session : null;
    	$sessionShop = $session ? $session->get('shop') : '[-]';
